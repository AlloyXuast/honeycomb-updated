...
Promise.all([config.startURL, config.clientURL]).then(urls => {
  ...
  for (var i = 0; i < config.CustomAPI.length; i++) {
    ...
    api.get(config.CustomAPI[i].path, config.CustomAPI[i].func)
  }
  ...
  function startApp() {
    ...
    for (var i = 0; i < config.CustomChron.length; i++) {
      chronOps[config.CustomChron[i].op] = config.CustomChron[i].func
    }
